...
exports.getMonthlyFeedback = async (req, res) => {
  ...
  db.query(sql, [user_id, month, prevMonthStr], async (err, rows) => {
    ...
    rows.forEach(row => {
      ...
      result[target][row.type] = row.total;
    });

    const summary = `
[이번 달]
- 수입: ${result.current.income.toLocaleString()}원
- 지출: ${result.current.expense.toLocaleString()}원

[지난 달]
- 수입: ${result.previous.income.toLocaleString()}원
- 지출: ${result.previous.expense.toLocaleString()}원
    `.trim();
