...
exports.getMonthlySummaryForGPT = (req, res) => {
  ...
  db.query(sql, [user_id, currentMonth, prevMonth], (err, rows) => {
    ...
    const summaryText = `
      - 이번 달(${currentMonth}) 수입: ${formatAmount(summary[currentMonth].income || 0)} / 지출: ${formatAmount(summary[currentMonth].expense || 0)}
      - 지난 달(${prevMonth}) 수입: ${formatAmount(summary[prevMonth].income || 0)} / 지출: ${formatAmount(summary[prevMonth].expense || 0)}
      `;
...
// 월별 총액 데이터 (최근 6개월) - 수입과 지출 모두 포함
exports.getMonthlyTotals = (req, res) => {
  const { user_id } = req.query;
  if (!user_id) {
    return res.status(400).json({ message: 'user_id는 필수입니다.' });
  }
